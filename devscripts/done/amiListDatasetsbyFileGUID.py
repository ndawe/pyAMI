...
class amiListDatasetsbyFileGUID:
    """
       A command to list datasets by fileGUID.The first argument must be a either \"-help\" or a fileGUID
    ...
    def command(self , argv):
        ...
        argument.append("glite=SELECT logicalDatasetName,files.LFN WHERE amiStatus='VALID'  AND files.fileGUID = '" + argv[0] + "'")
...
def main(argv):
    ...
    try:
        if(len(argv) > 0  and ((argv[0] == "-help")or (argv[0] == "help"))):
            print amiListDatasetsbyFileGUID.__doc__
            return None

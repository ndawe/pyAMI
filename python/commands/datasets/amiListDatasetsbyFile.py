...
class amiListDatasetsbyFile:
   ...
   def command(self ,  argv):
	  if len(argv)==0:
		    raise AMI_Error( "You must provide a filename" )
	  limit="0,10"
	  if(len(argv)>0  and argv[0]=="-help"):
	     print " A command to list the dataset which contains the given file."
	     print " The first argument must be a either \"-help\" or a file name. "
             print "  "
	  ...
	  argument.append("glite=SELECT logicalDatasetName,files.fileGUID WHERE amiStatus='VALID'  AND files.LFN = '"+argv[0]+"' LIMIT "+limit)

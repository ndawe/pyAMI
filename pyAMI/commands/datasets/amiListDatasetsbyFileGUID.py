...
class amiListDatasetsbyFileGUID:
   def __init__( self ):
      return 
   def command(self ,  argv):
	  if len(argv)==0:
		    raise AMI_Error( "You must provide a file GUID" )
	  limit="0,10"
	  if(len(argv)>0  and argv[0]=="-help"):
	     print " A command to list datasets by fileGUID."
	     print " The first argument must be a either \"-help\" or a fileGUID"
             print "  "
             print " Only valid datasets are returned."
	  ...
	  argument.append("glite=SELECT logicalDatasetName,files.LFN WHERE amiStatus='VALID'  AND files.fileGUID = '"+argv[0]+"' LIMIT "+limit)
...
def main(argv):
		...
		try:
			...
			amiclient=AMI()
			result= amiclient.execute(amiListDatasetsbyFileGUID().command(argv))		
			print result.output()
